...
# Configuración de conexión MySQL a Amazon RDS (actualizada)
app.config['MYSQL_HOST'] = 'paginaweb2.c1arstmsx1pg.us-east-1.rds.amazonaws.com'
app.config['MYSQL_USER'] = 'admin'
app.config['MYSQL_PASSWORD'] = 'Yancarlos123'
...
def admin_required(f):
    @wraps(f)
    def decorated_function(*args, **kwargs):
        print('DEBUG admin_required:', session.get('user_id'), session.get('user_type'))
        if 'user_id' not in session:
            print('DEBUG: No user_id in session')
            flash('Debes iniciar sesión para acceder a esta página.', 'error')
            return redirect(url_for('login'))
        if session.get('user_type') != 'admin':
            print('DEBUG: user_type is not admin:', session.get('user_type'))
...
def home():
    ...
    cur.execute("""
        SELECT p.id, p.nombre, p.precio, p.imagen, p.precio_original, p.descuento, 
               p.destacado, p.fecha_lanzamiento, c.nombre as categoria_nombre
        FROM productos p 
    ...
    for prod in productos_db:
        ...
        es_nuevo = False
        if prod['fecha_lanzamiento']:
            es_nuevo = prod['fecha_lanzamiento'] >= fecha_limite_nuevo.date()
...
def login():
    if request.method == 'POST':
        ...
        try:
            ...
            user = cur.fetchone()
            cur.close()
            print('DEBUG login: User from DB:', user)
            if user and check_password_hash(user['contrasena'], password):
                ...
                print('DEBUG login: Session set:', dict(session))
                if remember_me:
                ...
                if user['tipo_usuario'] == 'admin':
                    print('DEBUG login: Redirecting to admin_dashboard')
                    return redirect(url_for('admin_dashboard'))
                else:
                    print('DEBUG login: Redirecting to profile')
...
def contacto():
    if request.method == 'POST':
        ...
        try:
            cur = mysql.connection.cursor()
            cur.execute("""
                INSERT INTO contactos (nombre, apellido, email, asunto, mensaje)
                VALUES (%s, %s, %s, %s, %s)
            """, (nombre, apellido, email, asunto, mensaje))
            mysql.connection.commit()
            cur.close()
            flash('¡Mensaje enviado exitosamente! Pronto te responderemos.', 'success')
...
def carrito():
    if 'user_id' not in session:
        return render_template('carrito.html', productos=[], total=0.0)
    cur = mysql.connection.cursor()
    cur.execute('''
    ...
    for row in cur.fetchall():
        subtotal = float(row['precio']) * row['cantidad']
        productos.append({
            'id': row['id'],
            'nombre': row['nombre'],
            'precio': float(row['precio']),
            'imagen': row['imagen'] or 'producto_destacado.png',
...
def blog_detalle(blog_id):
    ...
    if request.method == 'POST':
        ...
        if comentario and 1 <= puntuacion <= 5:
            cur.execute("""
                INSERT INTO comentarios_blog (blog_id, correo_suscriptor, comentario, puntuacion)
                VALUES (%s, %s, %s, %s)
            """, (blog_id, usuario, comentario, puntuacion))
            mysql.connection.commit()
    ...
    cur.execute("SELECT correo_suscriptor as usuario, comentario, puntuacion FROM comentarios_blog WHERE blog_id = %s ORDER BY fecha DESC", (blog_id,))
...
def catalogo_categoria(nombre_categoria):
    mapeo_categorias = {
        ...
        'perfume': 'Perfume',      # <-- Singular, igual que en la base de datos
        'perfumes': 'Perfume',     # <-- Singular, igual que en la base de datos
        'joyeria': 'Joyería',
    ...
    # Ordenamiento
    if orden == 'fecha':
        query += " ORDER BY p.fecha_lanzamiento DESC"
...
def api_productos():
    ...
    query = """
        SELECT p.nombre, p.precio, p.imagen, p.id, p.precio_original, p.descuento, 
               p.fecha_lanzamiento, p.destacado, c.nombre as categoria_nombre
        FROM productos p 
    ...
    for prod in productos_db:
        ...
        productos.append({
            ...
            'precio_original': float(prod['precio_original']) if prod['precio_original'] else None,
            'descuento': int(prod['descuento']) if prod['descuento'] else None,
            'fecha_lanzamiento': prod['fecha_lanzamiento'].isoformat() if prod['fecha_lanzamiento'] else None,
            'categoria': prod['categoria_nombre'] or 'Sin categoría',
            'es_nuevo': es_nuevo,
            'es_top_seller': bool(prod['destacado'])
        })
...
def api_productos_categoria():
    ...
    for prod in productos_db:
        productos.append({
            ...
            'id': prod['id'],
            'categoria': prod['categoria_nombre'] or 'Sin categoría',
            'es_nuevo': False,  # Puedes calcularlo si lo necesitas
            'es_top_seller': bool(prod['destacado'])
        })
...
def mis_pedidos():
    ...
    cur = mysql.connection.cursor()
    cur.execute("SELECT id, fecha, total, estado, puntos_ganados FROM pedidos WHERE usuario_id = %s ORDER BY fecha DESC", (usuario_id,))
    pedidos_db = cur.fetchall()
    pedidos = []
    for pedido in pedidos_db:
        cur.execute("""
            SELECT p.nombre, dp.cantidad, dp.subtotal, p.imagen
        ...
        pedidos.append({
            ...
            'puntos': pedido.get('puntos_ganados', 0) or 0,
            'progreso': 100 if pedido['estado'] == 'Entregado' else 80 if pedido['estado'] == 'Enviado' else 40,
            'seguimiento': 'Entregado' if pedido['estado'] == 'Entregado' else 'En camino' if pedido['estado'] == 'Enviado' else 'Pendiente',
            'productos': productos
        })
    cur.close()
...
def api_carrito():
    ...
    cur = mysql.connection.cursor()  # No usar dictionary=True
    cur.execute('''
        SELECT p.id, p.nombre, p.precio, p.imagen, c.cantidad
        FROM carrito c
        JOIN productos p ON c.producto_id = p.id
...
def producto_detalle(producto_id):
    try:
        ...
        cur.execute("SELECT id, nombre, descripcion, precio, imagen, precio_original, descuento, categoria_id, tipo, acabado, material, genero, fecha_lanzamiento, peso, dimensiones FROM productos WHERE id = %s", (producto_id,))
        prod = cur.fetchone()
        print(f"Producto obtenido: {prod}")
        if not prod:
            cur.close()
        ...
        producto = {
            ...
            'beneficios': [],
            'tipo': prod['tipo'] or '',
            'acabado': prod['acabado'] or '',
            'material': prod['material'] or '',
            'genero': prod['genero'] or '',
            'fecha_lanzamiento': prod['fecha_lanzamiento'] or '',
            'peso': prod['peso'] if prod['peso'] is not None else 0,
            'dimensiones': prod['dimensiones'] or ''
        }
        ...
        print(f"Relacionados: {relacionados_db}")
        productos_relacionados = []
        for rel in relacionados_db:
            productos_relacionados.append({
                'id': rel['id'],
                'nombre': rel['nombre'],
                'precio': float(rel['precio']),
                'imagen_url': url_for('static', filename='img/' + (rel['imagen'] or 'producto_destacado.png')),
                'precio_original': float(rel['precio_original']) if rel['precio_original'] else None,
                'descuento': int(rel['descuento']) if rel['descuento'] else None,
            })
        cur.close()
        ...
        return "Error interno al mostrar el producto. Revisa la consola para más detalles.", 500
...
def admin_usuarios():
    ...
    try:
        cur = mysql.connection.cursor()
        cur.execute("""
            SELECT id, nombre, email, tipo_usuario, email_verificado, ultimo_login, fecha_registro, avatar_url
            FROM usuarios 
            ORDER BY id DESC
        """)
        usuarios = [
...
def admin_usuario_editar(id):
    ...
    try:
        cur = mysql.connection.cursor()
        cur.execute("""
            SELECT id, nombre, email, tipo_usuario, email_verificado, fecha_registro, avatar_url
            FROM usuarios WHERE id = %s
        """, (id,))
        ...
        usuario = {
            ...
            'avatar_url': usuario_data['avatar_url']
        }
        cur.close()
...
def actualizar_avatar():
    ...
    # Guardar el archivo en static/img/avatars/
    ...
    cur = mysql.connection.cursor()
    cur.execute("UPDATE usuarios SET avatar_url = %s WHERE id = %s", (avatar_url, session['user_id']))
    mysql.connection.commit()
    cur.close()
    ...
    return jsonify({'success': True, 'message': 'Avatar actualizado correctamente.', 'avatar_url': avatar_url})

@app.route('/api/comentarios_blog', methods=['GET', 'POST'])
def api_comentarios_blog():
    if request.method == 'POST':
        blog_id = request.form.get('blog_id', type=int)
        comentario = request.form.get('comentario', '').strip()
        puntuacion = int(request.form.get('puntuacion', 0))
        usuario = session.get('suscriptor_email', 'Anónimo')
        if not (blog_id and comentario and 1 <= puntuacion <= 5):
...
@app.route('/probar_email')
def probar_email():
    try:
        msg = Message(
            subject='Prueba de envío de correo desde Flask',
            sender=app.config['MAIL_USERNAME'],
            recipients=[app.config['MAIL_USERNAME']],
            body='¡Este es un correo de prueba enviado desde tu aplicación Flask!'
        )
        mail.send(msg)
        return 'Correo de prueba enviado correctamente a ' + app.config['MAIL_USERNAME']
    except Exception as e:
        return f'Error al enviar el correo: {e}'

@app.route('/hacerme_admin')
@login_required
def hacerme_admin():
    try:
        user_id = session['user_id']
        cur = mysql.connection.cursor()
        cur.execute("UPDATE usuarios SET tipo_usuario = 'admin' WHERE id = %s", (user_id,))
        mysql.connection.commit()
        cur.close()
        session['user_type'] = 'admin'
        flash('¡Ahora eres administrador!', 'success')
    except Exception as e:
        flash('Error al intentar hacerte admin: ' + str(e), 'error')
    return redirect(url_for('profile'))
